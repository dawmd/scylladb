...
sstring udt_to_str(const user_type_impl& udt) {
    ...
    auto bytes = std::move(*udt_desc.create_statement).as_managed_bytes();
...
future<> random_schema::create_with_cql(cql_test_env& env) {
    return async([this, &env] {
        ...
        auto bytes = std::move(*schema_desc.create_statement).as_managed_bytes();
        sstring create_statement = std::move(bytes).with_linearized([] (auto&& view) -> sstring {
            return sstring(to_string_view(view));
        });
