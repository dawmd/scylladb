...
def test_alter_index_if_not_exists(cql, test_keyspace, scylla_only):
    ...
    with new_test_table(cql, test_keyspace, 'p int primary key, v int') as table:
        try:
            ...
            with pytest.raises(InvalidRequest, match=f"Index {test_keyspace}.{nonexistent_index_name} does not exist"):
                cql.execute(f"ALTER INDEX {test_keyspace}.{nonexistent_index_name} WITH synchronous_updates = true")
        finally:
            cql.execute(f"DROP INDEX IF EXISTS {test_keyspace}.{ok_index_name}")
...
# Verify that creating a named index with simple view properties that are obsolete finishes
...
def test_create_index_simple_valid_obsolete_view_properties(cql, test_keyspace, scylla_only):
    with new_test_table(cql, test_keyspace, "p int PRIMARY KEY, v int, u int") as table:
        def do_test(property, value):
            ...
            assert hasattr(row_value, property)
            assert getattr(row_value, property) == None
...
# successfully, but that the option are NOT applied. In other words, the underlying MVs of
# indexes should behave the same way as regular MVs.
def test_alter_index_simple_valid_obsolete_view_properties(cql, test_keyspace, scylla_only):
    with new_test_table(cql, test_keyspace, "p int PRIMARY KEY, v int, u int") as table:
        index_name = unique_name()
        view_name = f"{index_name}_index"
...
def test_create_index_extensions(cql, test_keyspace, scylla_only):
    with new_test_table(cql, test_keyspace, "p int PRIMARY KEY, v int, u int") as table:
        def do_test(mode):
            ...
            cql.execute(f"CREATE INDEX {index_name} ON {table}(v) WITH tombstone_gc = {{'mode': '{mode}'}}")

            row_value = cql.execute(f"SELECT extensions FROM system_schema.views WHERE " \
                                    f"keyspace_name = '{test_keyspace}' AND view_name = '{view_name}'").one()

            assert hasattr(row_value, "extensions")
            extensions = row_value.extensions

            assert "tombstone_gc" in extensions
            assert extensions.get("tombstone_gc") == mode

        # FIXME: We don't run any test here because the `extensions` field in `system_schema.views`
        #        is represented by raw bytes and is not human-readable.
...
def test_alter_index_extensions(cql, test_keyspace, scylla_only):
    with new_test_table(cql, test_keyspace, "p int PRIMARY KEY, v int, u int") as table:
        ...
        def do_test(mode):
            cql.execute(f"ALTER INDEX {test_keyspace}.{index_name} WITH tombstone_gc = {{'mode': '{mode}'}}")
...
def test_try_alter_no_name(cql, test_keyspace, scylla_only):
    with new_test_table(cql, test_keyspace, 'p int PRIMARY KEY, v int, u int') as table:
        index_name = unique_name()
...
def test_alter_case_sensitive_index(cql, test_keyspace, scylla_only):
    with new_test_table(cql, test_keyspace, 'p int PRIMARY KEY, v int, u int') as table:
...
def test_alter_with_use_keyspace(cql, test_keyspace, scylla_only):
    with new_test_table(cql, test_keyspace, 'p int PRIMARY KEY, v int, u int') as table:
        index_name = unique_name()
        view_name = f"{index_name}_index"
