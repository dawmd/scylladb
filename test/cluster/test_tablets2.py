...
async def test_tablet_missing_data_repair(manager: ManagerClient):
    ...
    servers = [await manager.server_add(cmdline=cmdline, property_file={"dc": "dc1", "rack": f"r{i + 1}"}) for i in range(3)]
...
async def test_tablet_repair_history(manager: ManagerClient):
    logger.info("Bootstrapping cluster")
    servers = [await manager.server_add(property_file={"dc": "dc1", "rack": f"r{i + 1}"}) for i in range(3)]
...
async def test_tablet_repair_ranges_selection(manager: ManagerClient):
    logger.info("Bootstrapping cluster")
    servers = [await manager.server_add(property_file={"dc": "dc1", "rack": f"r{i + 1}"}) for i in range(2)]
