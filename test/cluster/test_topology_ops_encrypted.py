...
async def test_topology_ops_encrypted(request, manager: ManagerClient, tablets_enabled: bool, tmp_path):
    ...
    servers = [await manager.server_add(config=cfg, property_file={"dc": "dc1", "rack": f"r1"})]
    ...
    servers += [await manager.server_add(config=cfg, property_file={"dc": "dc1", "rack": f"r{i % rf}"}) for i in range(num_nodes)]
