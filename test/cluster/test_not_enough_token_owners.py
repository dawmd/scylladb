...
async def test_not_enough_token_owners(manager: ManagerClient):
    ...
    server_b = (await manager.servers_add(2, config={'join_ring': False}, property_file={"dc": "dcz", "rack": "rz"}))[0]
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'dc1': 2} AND tablets = { 'enabled': true }") as ks_name:
