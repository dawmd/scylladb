...
BOOST_AUTO_TEST_CASE(create_role_with_salted_hash_and_password) {
    BOOST_REQUIRE_THROW(
        cql3::util::do_with_parser("CREATE ROLE jane WITH SALTED HASH = 'something' AND PASSWORD = 'something';", cql3::dialect{}, std::mem_fn(&cql3_parser::CqlParser::query)),
        exceptions::syntax_exception);
}

BOOST_AUTO_TEST_CASE(create_role_with_password_and_salted_hash) {
    BOOST_REQUIRE_THROW(
        cql3::util::do_with_parser("CREATE ROLE jane WITH PASSWORD = 'something' AND SALTED HASH = 'something';", cql3::dialect{}, std::mem_fn(&cql3_parser::CqlParser::query)),
        exceptions::syntax_exception);
}
