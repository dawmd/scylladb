...
SEASTAR_THREAD_TEST_CASE(test_load_balancing_works_with_in_progress_transitions) {
  do_with_cql_env_thread([] (auto& e) {
    ...
    auto host2 = topo.add_node(node_state::normal, 1, endpoint_dc_rack{.dc = "dc1", .rack = "r1"});
    auto host3 = topo.add_node(node_state::normal, 2, endpoint_dc_rack{.dc = "dc1", .rack = "r2"});
...
SEASTAR_THREAD_TEST_CASE(test_load_balancing_with_random_load) {
  cql_test_config cfg{};
  // This test uses at most 2 racks, but the used RF varies between 2 and 4.
  // Because of that, we can't enable `rf_rack_valid_keyspaces` in this test.
  cfg.db_config->rf_rack_valid_keyspaces.set(false);
...
SEASTAR_THREAD_TEST_CASE(test_load_balancing_merge_colocation_with_decomission) {
    cql_test_config cfg{};
    // We cannot enable `rf_rack_valid_keyspaces` in this test because it purposefully uses just one rack.
    // Creating a keyspace with RF > 1 will NOT be possible.
    cfg.db_config->rf_rack_valid_keyspaces.set(false);
