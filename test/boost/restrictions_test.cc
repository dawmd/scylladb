...
SEASTAR_THREAD_TEST_CASE(tuple_of_list) {
    do_with_cql_env_thread([](cql_test_env& e) {
        ...
        require_rows(e, "select l1 from t where (l1,l2)>=([11,12],[101,102]) allow filtering",
            {{LI({11, 12})}, {LI({21, 22})}});
        require_rows(e, "select l1 from t where (l1,l2)<([11,12],[101,103]) allow filtering", {{LI({11, 12})}});
    }).get();
}
...
SEASTAR_THREAD_TEST_CASE(multi_col_slice) {
    do_with_cql_env_thread([](cql_test_env& e) {
        ...
        require_rows(e, "select p from t where (c1,c2)>=('a',20) and (c1,c2)<('b',3) allow filtering", {{I(2)}});
...
SEASTAR_THREAD_TEST_CASE(multi_col_slice_reversed) {
    do_with_cql_env_thread([](cql_test_env& e) {
        ...
        require_rows(e, "select c1 from t where (c1,c2)>(10,99) allow filtering", {{I(11)}, {I(12)}, {I(12)}});
...
SEASTAR_THREAD_TEST_CASE(multi_col_in) {
    do_with_cql_env_thread([](cql_test_env& e) {
        ...
        require_rows(e, "select ck1 from t where (ck1,ck2) in ((11,21),(12,22)) allow filtering", {{I(11)}, {I(12)}});
