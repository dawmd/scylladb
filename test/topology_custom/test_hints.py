...
async def test_draining_hints(manager: ManagerClient):
    ...
    await cql.execute_async("CREATE KEYSPACE ks WITH REPLICATION = {'class': 'SimpleStrategy', 'replication_factor': 3}")
    await cql.execute_async("CREATE TABLE ks.t (pk int PRIMARY KEY, v int)")
...
async def test_canceling_hint_draining(manager: ManagerClient):
    ...
    await cql.execute_async("CREATE KEYSPACE ks WITH REPLICATION = {'class': 'SimpleStrategy', 'replication_factor': 1}")
    await cql.execute_async("CREATE TABLE ks.t (pk int PRIMARY KEY, v int)")
