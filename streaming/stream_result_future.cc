    ...
    if (!has_active_sessions) {
        ...
        (void)_mgr.get_progress_on_all_shards(plan_id).then([duration, stats] (auto sbytes) {
            ...
            if (std::fabs(duration) > FLT_EPSILON) {
                tx_bw = seastar::format"{:.2f}", sbytes.bytes_sent / duration / 1024);
                rx_bw = seastar::format"{:.2f}", sbytes.bytes_received  / duration / 1024);
            }
            *stats = seastar::format"tx={:d} KiB, {} KiB/s, rx={:d} KiB, {} KiB/s", sbytes.bytes_sent / 1024, tx_bw, sbytes.bytes_received / 1024, rx_bw);
