...
void database::check_tablet_mvs_and_sis() const {
    if (!get_config().rf_rack_valid_keyspaces()) {
        for (const auto& view : get_views()) {
            dblog.debug("[check_tablet_mvs_and_sis]: Checking '{}.{}'", view->ks_name(), view->cf_name());

            if (view->table().uses_tablets()) {
                throw std::runtime_error(std::format(
                        "Materialized views/secondary indices with tablets can only be used with the option `rf_rack_valid_keyspaces` "
