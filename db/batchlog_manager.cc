...
future<size_t> db::batchlog_manager::count_all_batches() const {
    sstring query = seastar::format"SELECT count(*) FROM {}.{} BYPASS CACHE", system_keyspace::NAME, system_keyspace::BATCHLOG);
...
future<> db::batchlog_manager::replay_all_failed_batches(post_replay_cleanup cleanup) {
    ...
    co_await with_gate(_gate, [this, cleanup, batch = std::move(batch)] () mutable -> future<> {
        ...
        co_await _qp.query_internal(
                seastar::format"SELECT id, data, written_at, version FROM {}.{} BYPASS CACHE", system_keyspace::NAME, system_keyspace::BATCHLOG),
