...
public:
    send_one_file_ctx(map_type<table_schema_version, column_mapping>& last_schema_ver_to_column_mapping) noexcept
...
public:
    ...
    // Marked as noexcept because replay_position::operator< is noexcept.
    ...
    // Returns a position up to which hints have been successfully sent.
...
void hint_sender::notify_replay_waiters() noexcept {
    ...
    while (!_replay_waiters.empty() && _replay_waiters.begin()->first < _sent_upper_bound_rp) {
        manager_logger.trace("[{}] notify_replay_waiters(): notifying one ({} < {})",
                _host_id, _replay_waiters.begin()->first, _sent_upper_bound_rp);
...
bool hint_sender::send_one_file(const seastar::sstring& fname) {
    ...
    with_shared(*_host_manager.get_file_update_mutex_ptr(), [&fname, this] {
