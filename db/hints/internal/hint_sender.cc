...
future<::timespec> get_last_file_modification(const std::string_view fname) {
    seastar::file f = co_await open_file_dma(fname, open_flags::ro);
...
{
    // In either case, we won't save yet another hint as a result of sending this one.
        ...
        fragmented_temporary_buffer& buf)
{
    ...
    const auto& cm = get_column_mapping(std::move(ctx_ptr), fm, hr);
