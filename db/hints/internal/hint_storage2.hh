...
private:
    std::unique_ptr<host_hint_storage_impl> _impl;

public:
    host_hint_storage(std::unique_ptr<host_hint_storage_impl> impl)
        : _impl{std::move(impl)}
    {}
    ~host_hint_storage() = default;
...
private:
    ...
    // Scheduling group I/O operations performed by this object should belong to.
...
public:
    ...
    /// The resultant `host_hint_storage` inherits the scheduling group of this object.
    ///
    /// If this object's scheduling group is `std::nullopt`, the result will not be assigned
    /// to any scheduling group.
    host_hint_storage get_host_hint_storage_for(host_id_type host_id);

    /// Assign a specific scheduling group to the resultant object explicitly.
    /// It can be different to this object's scheduling group.
    host_hint_storage get_host_hint_storage_with_scheduling_group_for(host_id_type host_id,
            seastar::scheduling_group sched_group);

    template <typename Func>
        requires std::invocable<Func, host_id_type>
    decltype(auto) for_each_host_dir(Func func) {
