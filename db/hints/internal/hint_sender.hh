...
private:
    ...
    // Users can request a notification when hints up to some replay positions
    // have been sent. This multimap is for that purpose. It's a multimap
    // because multiple requests can be issued.
...
private:
    ...
    ///
    /// If sending fails we are going to set the state::segment_replay_failed in the _state and _first_failed_rp
    /// will be updated to min(_first_failed_rp, \ref rp).
    ///
    ...
    seastar::future<> send_one_hint(lw_shared_ptr<send_one_file_ctx> ctx_ptr, fragmented_temporary_buffer buf,
            replay_position rp, gc_clock::duration secs_since_file_mod,
