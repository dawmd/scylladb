...
{
    manager_logger.info("Trying to insert a new mapping: {} / {}", host_id, ip);
    ...
    manager_logger.info("Inserted a new mapping ({}, {}). New mappings: {}", host_id, ip, _mappings);
...
void hint_directory_manager::remove_mapping(const locator::host_id& host_id) noexcept {
    ...
    manager_logger.info("Erasing mapping for {}. New = {}", host_id, _mappings);
}
...
void hint_directory_manager::remove_mapping(const gms::inet_address& ip) noexcept {
    ...
    manager_logger.info("Erasing mapping for {}. New = {}", ip, _mappings);
}
...
void hint_directory_manager::clear() noexcept {
    manager_logger.info("Clearing mappings!");
    _mappings.clear();
}

void hint_directory_manager::print() const noexcept {
    manager_logger.info("HINT DIR MAPS: {}", _mappings);
}
