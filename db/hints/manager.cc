    ...
    , _resource_manager(res_manager)
{
    if (utils::get_local_injector().enter("decrease_hints_flush_period")) {
        internal::hints_flush_period = std::chrono::seconds{1};
    }
}
...
future<> manager::start(shared_ptr<service::storage_proxy> proxy_ptr, shared_ptr<gms::gossiper> gossiper_ptr) {
    ...
    return lister::scan_dir(_hints_dir, lister::dir_entry_types::of<directory_entry_type::directory>(), [this] (fs::path datadir, directory_entry de) {
        internal::host_id_type ep = internal::host_id_type(de.name);
        if (!check_dc_for(ep)) {
...
void manager::forbid_hints_for_hosts_with_pending_hints() {
    ...
    boost::for_each(_host_managers, [this] (auto& pair) {
        internal::host_manager& ep_man = pair.second;
        if (has_ep_with_pending_hints(ep_man.host_id())) {
            ep_man.forbid_hints();
        } else {
            ep_man.allow_hints();
        }
...
sync_point::shard_rps manager::calculate_current_sync_point(const std::vector<gms::inet_address>& target_hosts) const {
    ...
    for (auto addr : target_hosts) {
        ...
        if (it != _host_managers.end()) {
            const internal::host_manager& ep_man = it->second;
...
bool manager::have_ep_manager(internal::host_id_type ep) const noexcept {
    return _host_managers.contains(ep);
}

internal::host_manager& manager::get_host_manager(internal::host_id_type ep) {
    auto it = _host_managers.find(ep);
    if (it == _host_managers.end()) {
        manager_logger.trace("Creating an ep_manager for {}", ep);
        internal::host_manager& host_man = _host_managers.emplace(ep, internal::host_manager{ep, *this}).first->second;
        host_man.start();
        return host_man;
    }
    return it->second;
...
bool manager::store_hint(internal::host_id_type ep, schema_ptr s, lw_shared_ptr<const frozen_mutation> fm, tracing::trace_state_ptr tr_state) noexcept {
...
bool manager::too_many_in_flight_hints_for(internal::host_id_type ep) const noexcept {
...
bool manager::can_hint_for(internal::host_id_type ep) const noexcept {
...
future<> manager::change_host_filter(host_filter filter) {
    ...
    return with_gate(_draining_eps_gate, [this, filter = std::move(filter)] () mutable {
        return with_semaphore(drain_lock(), 1, [this, filter = std::move(filter)] () mutable {
            ...
            // for some of them
            return lister::scan_dir(_hints_dir, lister::dir_entry_types::of<directory_entry_type::directory>(), [this] (fs::path datadir, directory_entry de) {
                const internal::host_id_type ep = internal::host_id_type(de.name);
...
bool manager::check_dc_for(internal::host_id_type ep) const noexcept {
    try {
...
seastar::lw_shared_ptr<seastar::shared_mutex> manager::get_file_update_mutex_ptr(internal::host_id_type ep) {
    return _host_managers.at(ep).get_file_update_mutex_ptr();
}
        ...
        return with_semaphore(drain_lock(), 1, [this, endpoint] {
            return futurize_invoke([this, endpoint] () {
                if (utils::fb_utilities::is_me(endpoint)) {
                    ...
                    auto ep_manager_it = _host_managers.find(endpoint);
                    if (ep_manager_it != _host_managers.end()) {
...
hints_segments_map get_current_hints_segments(const sstring& hints_directory) {
...
void rebalance_segments(const sstring& hints_directory, hints_segments_map& segments_map) {
...
///
/// \param hints_directory a root hints directory
void remove_irrelevant_shards_directories(const sstring& hints_directory) {
    // shards level
