...
resource_manager::get_send_units_for(size_t buf_size) {
    ...
    return seastar::get_units(_send_limiter, hint_memory_budget);
}
        ...
        host_id_type ep_key)
{
    ...
    auto lambda = [&] (fs::path dir, seastar::directory_entry de) -> seastar::future<> {
        ...
        _total_size += co_await io_check(seastar::file_size, path.c_str());
    };
...
seastar::future<> resource_manager::register_manager(manager& m) {
    const auto operation_units = co_await seastar::get_units(_operation_lock, 1);
    const auto update_units = co_await seastar::get_units(_space_watchdog.update_lock(), 1);
    ...
    // If the resource_manager has started, start the hint manager too.
    try {
...
// TODO: Check if this refactor was correct indeed.
seastar::future<> resource_manager::prepare_per_device_limits(manager& shard_manager) {
    ...
    if (it != _per_device_limits_map.end()) {
        ...
        auto [empl_it, inserted] = _per_device_limits_map.emplace(device_id,
                space_watchdog::per_device_limits{});
        ...
        if (inserted) {
            ...
            pdl.max_shard_disk_space_size = std::filesystem::space(
                    shard_manager.hints_dir().c_str()).capacity / (10 * smp::count);

            // If hints directory is a mountpoint, we assume it's on dedicated
            // (i.e. not shared with data/commitlog/etc) storage.
