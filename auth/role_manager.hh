...
public:
    ...
    /// Produces descriptions that can be used to restore the roles.
    virtual future<std::vector<cql3::description>> describe_roles(bool with_salted_hashes) const = 0;

    /// Produces descriptions that can be used to restore the role grants.
    virtual future<std::vector<cql3::description>> describe_role_grants() const = 0;

    /// Produces descriptions that can be used to restore the attached service levels.
    virtual future<std::vector<cql3::description>> describe_attached_service_levels() const = 0;
};
}
