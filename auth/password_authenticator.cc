...
future<authenticated_user> password_authenticator::authenticate(
    ...
    try {
        const std::optional<sstring> salted_hash = co_await query_salted_hash(username);
...
bool password_authenticator::uses_salted_hashes() const {
    return true;
}

future<std::optional<sstring>> password_authenticator::query_salted_hash(std::string_view role_name) const {
