...
future<std::vector<cql3::description>> service::describe_roles(bool with_salted_hashes) const {
    std::vector<cql3::description> result{};

    const auto roles = co_await _role_manager->query_all();
    result.reserve(roles.size());

    const bool authenticator_uses_salted_hashes = _authenticator->uses_salted_hashes();

    auto maybe_fetch_salted_hash = [&] (std::string_view role_name) -> future<std::optional<sstring>> {
        if (authenticator_uses_salted_hashes) {
            co_return co_await _authenticator->query_salted_hash(role_name);
        }
    ...
    for (const auto& role : roles) {
        ...
        const std::optional<sstring> maybe_salted_hash = co_await maybe_fetch_salted_hash(role);
...
future<std::vector<cql3::description>> service::describe_permissions() const {
    ...
    const auto permission_list = co_await std::invoke([&] -> future<std::vector<permission_details>> {
        try {
            ...
            // will throw an exceptoion when trying to access them.
