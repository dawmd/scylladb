...
future<> storage_service::join_cluster(sharded<db::system_distributed_keyspace>& sys_dist_ks, sharded<service::storage_proxy>& proxy,
        sharded<gms::gossiper>& gossiper, bool start_hint_manager) {
    assert(this_shard_id() == 0);
    ...
    co_return co_await join_token_ring(sys_dist_ks, proxy, gossiper, std::move(initial_contact_nodes),
            std::move(loaded_endpoints), std::move(loaded_peer_features), get_ring_delay(), start_hint_manager);
}
...
future<> storage_service::excise(std::unordered_set<token> tokens, inet_address endpoint, locator::host_id hid, gms::permit_id pid) {
    ...
    co_await notify_left(endpoint, {hid});
}

future<> storage_service::excise(std::unordered_set<token> tokens, inet_address endpoint, locator::host_id hid, int64_t expire_time, gms::permit_id pid) {
    add_expire_time_if_found(endpoint, expire_time);
    return excise(tokens, endpoint, hid, pid);
}
...
future<> endpoint_lifecycle_notifier::notify_left(gms::inet_address endpoint, std::optional<locator::host_id> maybe_hid) {
    return seastar::async([this, endpoint, maybe_hid] {
        _subscribers.thread_for_each([endpoint, maybe_hid] (endpoint_lifecycle_subscriber* subscriber) {
            try {
                subscriber->on_leave_cluster(endpoint, maybe_hid);
            } catch (...) {
...
future<> storage_service::notify_left(inet_address endpoint, std::optional<locator::host_id> maybe_hid) {
    co_await container().invoke_on_all([endpoint, maybe_hid] (auto&& ss) {
        return ss._lifecycle_notifier.notify_left(endpoint, maybe_hid);
    });
