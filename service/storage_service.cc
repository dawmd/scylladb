...
future<> storage_service::topology_state_load() {
        ...
        for (const auto& [id, rs]: _topology_state_machine._topology.transition_nodes) {
            ...
            case node_state::replacing: {
                if (rs.ring.has_value()) {
                    ...
                    if (ip == existing_ip) {
                        tmptr->update_topology(replaced_host_id,
                            locator::endpoint_dc_rack{rs.datacenter, rs.rack},
                            locator::node::state::being_replaced, rs.shard_count);
                        tmptr->update_host_id(replaced_host_id, ip);
                    }
                    update_topology(host_id, ip, rs);
    ...
    for (const auto& e: tmptr->get_all_endpoints()) {
        ...
        if (e != tmptr->get_my_id() && !_gossiper.get_endpoint_state_ptr(ep)) {
...
future<> storage_service::handle_state_normal(inet_address endpoint, gms::permit_id pid) {
    ...
    if (existing && *existing != endpoint) {
        ...
        tmptr->del_replacing_endpoint(host_id);
    } else {
        ...
        if (const auto old_host_id = tmptr->get_host_id_if_known(endpoint); old_host_id) {
            tmptr->del_replacing_endpoint(*old_host_id);
        }
...
future<> storage_service::join_cluster(sharded<db::system_distributed_keyspace>& sys_dist_ks, sharded<service::storage_proxy>& proxy, bool enable_hh) {
    assert(this_shard_id() == 0);
    ...
    co_return co_await join_token_ring(sys_dist_ks, proxy, std::move(initial_contact_nodes), std::move(loaded_endpoints),
            std::move(loaded_peer_features), get_ring_delay(), enable_hh);
}
...
future<node_ops_cmd_response> storage_service::node_ops_cmd_handler(gms::inet_address coordinator, locator::host_id coordinator_host_id, node_ops_cmd_request req) {
        ...
        if (req.cmd == node_ops_cmd::removenode_prepare) {
            ...
            mutate_token_metadata([coordinator, coordinator_host_id, &req, this] (mutable_token_metadata_ptr tmptr) mutable {
                for (auto& x: req.replace_nodes) {
                    ...
                    tmptr->update_topology(existing_node_id, get_dc_rack_for(existing_node), locator::node::state::being_replaced);
        ...
        if (ss._raft_topology_change_enabled) {
            ...
            if (ss.is_repair_based_node_ops_enabled(streaming::stream_reason::rebuild)) {
                ...
                auto streamer = make_lw_shared<dht::range_streamer>(ss._db, ss._stream_manager, tmptr, ss._abort_source,
                        tmptr->get_my_id(), ss._snitch.local()->get_location(), "Rebuild", streaming::stream_reason::rebuild);
...
future<> storage_service::removenode_with_stream(gms::inet_address leaving_node, shared_ptr<abort_source> as_ptr) {
    return seastar::async([this, leaving_node, as_ptr] {
        ...
        auto streamer = make_lw_shared<dht::range_streamer>(_db, _stream_manager, tmptr, as, tmptr->get_my_id(), _snitch.local()->get_location(), "Removenode", streaming::stream_reason::removenode);
...
storage_service::stream_ranges(std::unordered_map<sstring, std::unordered_multimap<dht::token_range, inet_address>> ranges_to_stream_by_keyspace) {
    auto streamer = dht::range_streamer(_db, _stream_manager, get_token_metadata_ptr(), _abort_source, get_token_metadata_ptr()->get_my_id(), _snitch.local()->get_location(), "Unbootstrap", streaming::stream_reason::decommission);
                    ...
                    co_await retrier(_rebuild_result, [&] () -> future<> {
                        ...
                        if (is_repair_based_node_ops_enabled(streaming::stream_reason::rebuild)) {
                            ...
                            auto streamer = make_lw_shared<dht::range_streamer>(_db, _stream_manager, tmptr, _abort_source,
                                    tmptr->get_my_id(), _snitch.local()->get_location(), "Rebuild", streaming::stream_reason::rebuild);
...
void storage_service::init_messaging_service(bool raft_topology_change_enabled) {
    _messaging.local().register_node_ops_cmd([this] (const rpc::client_info& cinfo, node_ops_cmd_request req) {
        ...
        auto coordinator_host_id = cinfo.retrieve_auxiliary_opt<locator::host_id>("host_id");
