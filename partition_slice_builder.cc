...
partition_slice_builder::with_regular_column(bytes name) {
    ...
    if (!def) {
        throw std::runtime_error(fmt::format"No such column: {}", _schema.regular_column_name_type()->to_string(name)));
    }
    if (!def->is_regular()) {
        throw std::runtime_error(fmt::format"Column is not regular: {}", _schema.column_name_type(*def)->to_string(name)));
    }
...
partition_slice_builder::with_static_column(bytes name) {
    ...
    const column_definition* def = _schema.get_column_definition(name);
    if (!def) {
        throw std::runtime_error(fmt::format"No such column: {}", utf8_type->to_string(name)));
    }
    if (!def->is_static()) {
        throw std::runtime_error(fmt::format"Column is not static: {}", utf8_type->to_string(name)));
    }
