...
view_ptr get_view_for_index(const data_dictionary::database& db, const sstring& ks, const sstring& index_name) {
    ...
    if (!result->is_view()) {
        throw exceptions::invalid_request_exception(seastar::format(
                "Secondary index {}.{} does not exist or it does not have the underlying materialized view",
                ks, index_name));
    }
...
// Requires that `view` points to a valid materialized view.
// The result is guaranteed to be a pointer to a valid `schema` object.
schema_ptr get_base_table_for_view(const data_dictionary::database& db, const view_ptr view) {
    const auto id = view->view_info()->base_id();
    return db.find_schema(id);
}
...
future<> alter_index_statement::check_access(query_processor& qp, const service::client_state& state) const {
    const view_ptr s = get_view_for_index(qp.db(), keyspace(), column_family());
    return state.has_column_family_access(s->ks_name(), s->view_info()->base_name(), auth::permission::ALTER);
}

void alter_index_statement::validate(query_processor& qp, const service::client_state& state) const {
    // This will throw if the underlying materialized view does not exist.
    const view_ptr view_schema_ptr = get_view_for_index(qp.db(), keyspace(), column_family());

    const schema_ptr base_table_ptr = get_base_table_for_view(qp.db(), view_schema_ptr);
    const bool is_index = std::ranges::contains(base_table_ptr->index_names(), column_family());

    if (!is_index) {
        throw exceptions::invalid_request_exception(seastar::format(
                "Index {}.{} does not exist", keyspace(), column_family()));
    }
