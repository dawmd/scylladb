...
static unsigned parse_rf(const std::string& rf) {
    try {
        return std::stoi(rf);
    } catch (std::invalid_argument const& ex) {
        on_internal_error(mylogger, std::format("parse_rf expects integer arguments, but got: {}", rf));
    } catch (std::out_of_range const& ex) {
        on_internal_error(mylogger, std::format("parse_rf expects integer arguments to fit into `int` type, but got: {}", rf));
    }
}
            ...
            if (ks.get_replication_strategy().uses_tablets()) {
                ...
                for (const auto& [new_dc, new_rf] : new_rf_per_dc) {
                    ...
                    if (auto new_dc_in_current_mapping = current_rf_per_dc.find(new_dc);
                        ...
                        old_rf = parse_rf(new_dc_in_current_mapping->second);
