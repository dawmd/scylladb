...
future<std::vector<description>> table(const data_dictionary::database& db, const sstring& ks, const sstring& name, bool with_internals) {
    ...
    if (cdc::is_log_for_some_table(db.real_database(), ks, name)) {
        ...
        managed_bytes bytes = std::move(*table_desc.create_statement).as_managed_bytes();
        auto view = managed_bytes_view(bytes);

        while (!view.empty()) {
            std::basic_string_view<int8_t> fragment = view.current_fragment();
            std::string_view sv = {reinterpret_cast<const char*>(fragment.data()), fragment.size()};
