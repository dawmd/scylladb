...
void create_keyspace_statement::validate(query_processor& qp, const service::client_state& state) const
{
    ...
    const bool restricted_keyspaces = cfg.rf_rack_restricted_keyspaces.is_set();
    // At this point, we don't know yet if the replication strategy is NetworkTopologyStrategy!
    if (restricted_keyspaces && rs->get_type() == locator::replication_strategy_type::network_topology && rs->uses_tablets()) {
