...
{
    if (_raw_targets.empty() && !_properties->idx_opts.is_custom) {
...
std::vector<::shared_ptr<index_target>> create_index_statement::validate_while_executing(data_dictionary::database db) const {
    ...
    if (_properties && _properties->idx_opts.custom_class) {

        auto validator = secondary_index::secondary_index_manager::get_custom_class_factory(*_properties->idx_opts.custom_class);
        if (!validator) {
            throw exceptions::invalid_request_exception(format("Non-supported custom class \'{}\' provided", *_properties->idx_opts.custom_class));
        }
        (*validator)()->validate(*schema, _properties->idx_opts, targets, db.features());
    }
...
{
    if (!_properties->idx_opts.is_custom) {
...
std::optional<create_index_statement::base_schema_with_new_index> create_index_statement::build_index_schema(data_dictionary::database db) const {
    ...
    if (_properties->idx_opts.custom_class) {
        index_options = _properties->idx_opts.get_options();
